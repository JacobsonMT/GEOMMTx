...
public class CheckHighLevelSpreadSheet extends CreateSpreadSheet {
    ...
    public void populate( Map<String, Set<String>> newPredictions, Map<String,String> labels, int size ) throws Exception {
        ...
        for ( int i = 0; i < size; i++ ) {
            ...
            for ( String URL : newPredictions.get( dataset ) ) {
                row++;
                ExcelUtil.setFormula( spreadsheet, row, schema.getPosition( "Dataset" ), "HYPERLINK(\"http://bioinformatics.ubc.ca/Gemma/expressionExperiment/showExpressionExperiment.html?id="+dataset+"\",\""+ dataset+"\")");
    ...
    public static void main( String[] args ) throws Exception {
        // CheckHighLevelSpreadSheet test = new CheckHighLevelSpreadSheet( "ttx.xls" );
        // test.save();
        // log.info( "Done!" );
